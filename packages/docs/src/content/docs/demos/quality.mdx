...
|   | Method                                                      | Description                                                                                                                                                    |
|:--|-------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------|
|   | `getLevel(): Promise<number>`                               | Returns -1 for automatic level switching or returns the level requested by the user in the player.                                                             |
|   | `getLevels(): Promise<Array<{key: string; value: number}>>` | Returns the available levels.                                                                                                                                  |
|   | `getPlaybackLevel(): Promise<number>`                       | Returns the actual level used by HLS.js for playback. The value will be equal to getLevel() for fixed levels and different if the requested level is -1 (auto) |
